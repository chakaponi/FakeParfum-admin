...
var index = defineEndpoint({
  ...
  handler: async (router, context) => {
    ...
    router.post("/create", async (req, res) => {
      ...
      const purchaseItemsForInsert = await purchaseItems.map(async (item) => {
        const { product_id, size, quantity } = item;
        const { price } = purchaseProducts.find((product) => product.product_id === product_id).sizes_and_prices.find((sizes_and_prices) => sizes_and_prices.size === size);
